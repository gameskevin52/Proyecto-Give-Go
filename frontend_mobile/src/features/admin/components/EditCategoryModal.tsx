...
import { X, Edit3, Tag } from 'lucide-react-native';
...
interface EditCategoryModalProps {
  ...
  onClose: () => void;
  onSubmit: (id: string, payload: Partial<AdminCategoryItem>) => Promise<{ success: boolean; message?: string }>;
}
...
export const EditCategoryModal: React.FC<EditCategoryModalProps> = ({
  ...
  useEffect(() => {
    if (category) {
      ...
      setEstado((category.estado === 'inactivo' || category.estado === '0' || category.estado === 0) ? 'inactivo' : 'activo');
      setErrorMessage('');
    }
  }, [category]);
  ...
  const handleSubmit = async () => {
    ...
    try {
      setIsLoading(true);
      const catId = String(category.id || category.id_categoria);
      const res = await onSubmit(catId, {
        nombre: nombre.trim(),
      ...
      if (res.success) {
        Alert.alert('Éxito', res.message || 'Categoría actualizada correctamente.');
        onClose();
      } else {
        setErrorMessage(res.message || 'No se pudo actualizar la categoría.');
      }
    } catch (err: any) {
          ...
          <View style={styles.header}>
            <View style={{ flexDirection: 'row', alignItems: 'center', gap: 10 }}>
              ...
              <View>
                ...
                <Text style={styles.headerSubtitle}>ID: #{category.id || category.id_categoria}</Text>
              </View>
            </View>
